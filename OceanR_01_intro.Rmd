...
Let's go ahead and download the package 'data.table'.
  
After installing 'gdata' in our library, we need to tell R to make it available for use. 
```{r}
library(data.table)
```
Now we can learn about the package by using the help command, which will bring up the help page in the lower right panel of RStudio
```{r}
# get the help page for a package
??data.table
```
  
If we need help using a particular function, we use a similar command, except with a single '?'. Again, the help page will show up in the lower right panel of RStudio. Remember, this will only work if the package containing the function is loaded (unless it is a 'base' package)
```{r}
# get the help page for the 'mean' function
?mean()
```
